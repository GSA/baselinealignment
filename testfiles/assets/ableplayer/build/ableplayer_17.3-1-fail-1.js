		...
		for (i=0; i<available.length; i++) {
			...
			if ((available[i]['group'] == form) && available[i]['label']) {
				...
				if (form === 'captions') {
					...
					//Apply change on the Caption section
...
(function ($) {
	...
	AblePlayer.prototype.createPopup = function (which, tracks) {
		...
		if (which === 'prefs') {
			//Added custom button for Audio description
			this.prefCats.push("ad")
      if (this.prefCats.length > 1) {
			console.log(this.prefCats)
          ...
          if (prefCat === 'ad') {
					  $menuItem.text(this.tt.prefMenuAudioDescription);
				  }
          ...
          $menuItem.on('click',function() {
			...
			//Set this button to click on the original Audio Description button that hidden by CSS.
            else if (whichPref === thisObj.tt.prefMenuAudioDescription) {
						document.getElementsByClassName('able-button-handler-descriptions')[0].click();
					  }
            thisObj.closePopups();
