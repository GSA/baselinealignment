...
(function ($) {
	...
	AblePlayer.prototype.createPopup = function (which, tracks) {
		...
		if (which === 'prefs') {
			//Added custom button for caption
			this.prefCats.push("cn")
      if (this.prefCats.length > 1) {
		...
		// 		  }
          if (prefCat === 'cn') {
					  $menuItem.text(this.tt.prefMenuCaptionsOn);
				  }
          ...
          $menuItem.on('click',function() {
			...
			//Set this button to click on the original Captions button that hidden by CSS.
            else if (whichPref === thisObj.tt.prefMenuCaptionsOn) {
						document.getElementsByClassName('able-button-handler-captions')[0].click();
					  }
            thisObj.closePopups();
