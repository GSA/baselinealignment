...
# Baseline test
descr: | 
    The `<th>` element defines a table header. When there is no `<th>` elements, it is confusing what the column is representing. Table in the sample code includes a data table where there are no `<th>` elements.
