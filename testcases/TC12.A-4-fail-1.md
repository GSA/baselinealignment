---
layout: testcase
title: Data table with no `<th>` elements
tcid: TC12.A-4-fail-1
tc-version: "1"
...
descr: | 
    The `<th>` element defines a table header. When there are no `<th>` elements, it is confusing what the column is representing. Table in the sample code includes a data table where there are no `<th>` elements.
result: FAIL
result-descr: The table fails the baseline test because the data tables do not have header cells, `<th>`.
sample: /testfiles/TF12/12.A-4-fail-1.html
---
