...
| **1.A-1** | Check that all functionality can be accessed and executed using only the keyboard. [SC 2.1.1] | Pass - all functionality can be accessed and executed using only the keyboard |
| **1.A-2** | Check that individual keystrokes do not require specific timings for activation. [SC 2.1.1] | Fail - changing the Request Password Reset button to Request User Name requires the user to hold the spacebar for three seconds. |
| **1.A-2a** | 	If operation requires specific timings of individual keystrokes, check if another control is provided on the page with the same functionality which does not require specific timings for operation. | Fail: there is no alternative control that does not require specific timings for operation. |
| **Result** | If any of the above checks fail, then Baseline Test 1.A-KeyboardAccess fails. | Fail - 1.A-2 check fails. |
