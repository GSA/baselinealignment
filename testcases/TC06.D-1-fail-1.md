---
layout: testcase
title: 'Images of Text - FAIL: image of text that could easily be just text'
tcid: TC06.D-1-fail-1
tc-version: "1"
baseline-version: "3.1"
standard: WCAG
standard-ref: 1.4.5
standard-ref-url: https://www.w3.org/WAI/WCAG22/quickref/#images-of-text
descr: | 
    The code sample presents a page with an image of text that could easily be just text. A successful test should identify a FAIL against Baseline Test 6.4 Images of Text.
result: FAIL
result-descr: FAIL since there is an image of text that could easily be just text.
sample: /testfiles/TF06/06.D-1-fail-1.html
---
| Test Instruction | Instruction Detail | Expected Test Case Result |
|------------------|--------------------|---------------------------|
| **IC-1** | Identify any images of text, except where a particular presentation of text is essential to the information being conveyed (e.g., logotypes or text that is part of a logo or brand name). | The page includes an image of text. |
| **6.D-1** | Check that using text cannot achieve the same visual presentation and effect as images of text. `[SC 1.4.5]` | FAIL: using text could achieve the same visual presentation and effect. | 
| **Test Results** | If any of the above checks fail, then Baseline Test 6.D-ImageText fail. | FAIL; the page has an image of text that could easily be just text. |