...
| **6.1-1** | Calculate the accessible name and accessible description output for the image according to [HTML Accessibility API Mappings 1.0 for img](https://www.w3.org/TR/html-aam-1.0/#img-element). | The output for the accessible name and accessible description computations is `empty`. |
| **Result** | If any of the above checks fail, then Baseline Test 6.1-MeaningfulImage fails. | The first check fails due to empty accessible name and/or description in the text alternative computation output. |